...
@section('content')
    <h3 class="page-title">Kategori Soal</h3>

    <p>
        <a href="{{ route('topics.create') }}" class="btn btn-success">Tambah Kategori</a>
    </p>
                    ...
                    @if (count($topics) > 0)
                        @foreach ($topics as $topic)
                            <tr data-entry-id="{{ $topic->id }}">
                                ...
                                <td>{{ $topic->title }}</td>
                                <td>
                                    <a href="{{ route('topics.show',[$topic->id]) }}" class="btn btn-xs btn-primary">Detail</a>
                                    <a href="{{ route('topics.edit',[$topic->id]) }}" class="btn btn-xs btn-info">Edit</a>
                                    {!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('Apakah anda yakin menghapus kreteria ini');",
